...
# NEXT_PUBLIC_SITE_URL → canonical/OG URLs, robots.txt and sitemap.xml. Set it to
# your real domain so the sitemap points crawlers at the right place.
...
# Tesseract for editing scanned PDFs. The Document Translator runs Argos packs via
# ctranslate2 + sentencepiece (no argostranslate pkg → no stanza/PyTorch);
# fast-langdetect detects the source language.
RUN pip3 install --no-cache-dir --break-system-packages \
       pdf2docx==0.5.8 python-pptx==1.0.2 pdfplumber==0.11.4 openpyxl==3.1.5 \
       pytesseract==0.3.13 Pillow==11.0.0 ctranslate2 sentencepiece fast-langdetect
# Bundle the core Argos language packs (the "language bank") + warm the detector.
...
COPY services/convert/install_langs.py /tmp/install_langs.py
RUN mkdir -p "$ARGOS_PACKAGES_DIR" "$FTLANG_CACHE" \
  && python3 /tmp/install_langs.py \
...
# Admin/analytics store is embedded SQLite at PDFSHELL_DB_PATH. node:sqlite needs
# --experimental-sqlite on Node 22. Mount a disk on /app/data to persist it
# (without one it works but resets on each deploy/restart).
